...
public class ExplosionUtil {
    ...
    public static void doExplosionB(World world, Explosion explosion, boolean p_77279_1_) {
       ...
       /* world.playSoundEffect(explosion.explosionX, explosion.explosionY, explosion.explosionZ, "random.explode", 4.0F,
                (1.0F + (world.rand.nextFloat() - world.rand.nextFloat()) * 0.2F) * 0.7F);

        if (explosion.explosionSize >= 2.0F && explosion.isSmoking) {
            world.spawnParticle("hugeexplosion", explosion.explosionX, explosion.explosionY, explosion.explosionZ, 1.0D,
                    0.0D, 0.0D);
        } else {
            world.spawnParticle("largeexplode", explosion.explosionX, explosion.explosionY, explosion.explosionZ, 1.0D,
                    0.0D, 0.0D);
        }
        ...
        if (explosion.isSmoking) {
            ...
            while (iterator.hasNext()) {
                ...
                if (p_77279_1_) {
                    ...
                    d4 *= d7;
                    d5 *= d7;
                    world.spawnParticle("explode", (d0 + explosion.explosionX * 1.0D) / 2.0D,
                            (d1 + explosion.explosionY * 1.0D) / 2.0D, (d2 + explosion.explosionZ * 1.0D) / 2.0D, d3,
                            d4, d5);
                    ...
                    if (block.getMaterial() != Material.air) {
                        if (block.canDropFromExplosion(explosion)) {
                            if (mod_Invasion.getDestructedBlocksDrop()) {
                                block.dropBlockAsItemWithChance(world, i, j, k, world.getBlockMetadata(i, j, k),
                                        1.0F / explosion.explosionSize, 0);
                            } else {
                                block.dropBlockAsItemWithChance(world, i, j, k, world.getBlockMetadata(i, j, k), 0.0F,
                                        0);
                            }
                        }
