...
public class NavigatorIM implements INotifyTask, INavigation {
    ...
    public PathAction getCurrentWorkingAction() {
	    return !nodeActionFinished && !noPath() ? activeNode.action : PathAction.NONE;
	}
	...
	protected void updateAutoPathToEntity() {
		if (pathEndEntity != null && (noPath() || (pathEndEntity.getPos().distanceTo(pathEndEntityLastPos) / (6 + theEntity.getPos().distanceTo(pathEndEntityLastPos)) > 0.1D))) {
