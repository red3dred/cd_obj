...
import com.invasion.client.render.entity.model.ModelBigBiped;
...
public class RenderIMZombie extends BipedEntityRenderer<AbstractIMZombieEntity, BipedEntityModel<AbstractIMZombieEntity>> {
    ...
    protected final ModelBigBiped<AbstractIMZombieEntity> bigModel;

    public RenderIMZombie(EntityRendererFactory.Context ctx) {
        super(ctx, new ZombieEntityModel(ctx.getPart(EntityModelLayers.ZOMBIE)), 0.5F);
        normalModel = model;
        bigModel = new BigZombieEntityModel(ModelBigBiped.getTexturedModelData(Dilation.NONE, 0).createModel());
        ...
        addFeature(new ArmorFeature(this,
                new BigZombieEntityModel(ModelBigBiped.getTexturedModelData(new Dilation(0.5F), 0).createModel()),
                new BigZombieEntityModel(ModelBigBiped.getTexturedModelData(new Dilation(1), 0).createModel()),
                ctx.getModelManager(),
                true
        ));
    }
    ...
    private static final class BigZombieEntityModel extends ModelBigBiped<AbstractIMZombieEntity> {
